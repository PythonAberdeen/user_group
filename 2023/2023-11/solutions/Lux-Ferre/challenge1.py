...
def max_integer(a: int, b: int) -> int:
    if a >= b:
        return a
    else:
        return b
...
def print_if_42(value: int | str) -> None:
    if int(value) == 42:
        print('yay!')
    else:
        print('aww :(')
...
def first_two_unique_as_a_set(values: list[int]) -> set[int]:
    result = set()
    for value in values:
        result.add(value)
        if len(result) == 2:
...
# return an int
def now_many_nones(values: list[str|None]) -> int:
    result = 0
    for value in values:
        if value is None:
            result += 1
